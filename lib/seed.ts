...
export function buildSeedData(): AppData {
  ...
  const monthlyExpenses = (year: number, month: number): Transaction[] => {
    ...
    txs.push(makeTxOnDate(makeDate(year, month, 2), 120, "food", "Lunch at campus", {mood: "necessary", paymentMethod: "cash", time: "13:15" }))
    txs.push(makeTxOnDate(makeDate(year, month, 5), 350 + (month * 10), "food", "Groceries", { mood: "necessary", paymentMethod: "bkash" }))
    txs.push(makeTxOnDate(makeDate(year, month, 14), 180 + (month * 5), "food", "Dinner out", { mood: "luxury" }))
    ...
    txs.push(makeTxOnDate(makeDate(year, month, 4), 200, "internet", "Broadband bill", {  mood: "necessary", recurring: true, paymentMethod: "bkash" }))

   
    txs.push(makeTxOnDate(makeDate(year, month, 6), 1500, "parents", "Sent to parents", { mood: "necessary", paymentMethod: "bkash" }))

   
    if (month % 2 === 0) {
      txs.push(makeTxOnDate(makeDate(year, month, 12), 500 + (month * 20), "shopping", "Clothing purchase", {  mood: "regret", paymentMethod: "card" }))
    } else {
      txs.push(makeTxOnDate(makeDate(year, month, 16), 300, "shopping", "Household items", {  mood: "necessary", paymentMethod: "bkash" }))
    }

  
    txs.push(makeTxOnDate(makeDate(year, month, 18), 250 + (month * 8), "entertainment", "Movie night", { mood: "happy", paymentMethod: "card" }))
    ...
    if (month % 3 === 0) {
      txs.push(makeTxOnDate(makeDate(year, month, 15), 400, "medical", "Medicine", {  mood: "necessary", paymentMethod: "bkash" }))
    }
